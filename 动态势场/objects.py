...
class Obstacle:
    ...
    def __init__(self, position, mu=1, sigma=1, draw_radius=40, draw_color=(255,99,71),direction=1):
    ...
    def get_repulsion_force_pre(self,position):
        ...
        position_list_cal = [predict_position,Position(predict_position.x+20,predict_position.y),Position(predict_position.x,predict_position.y+20),Position(predict_position.x+20,predict_position.y+20)]
        dist_value = 0
        for Ever_position in position_list_cal:
            dist_value += (1/(self._sigma*math.sqrt(2*math.pi))) * math.exp(-(Position.calculate_distance_squared(Ever_position,position)/(2*self._sigma*self._sigma)))
